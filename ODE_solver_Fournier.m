...
ionstrength = 0.5 * (initialSubstance.ClO4 + substance(1) + 2 * substance(2) + 2 * substance(3) + 2 * substance(4) + substance(6)) / v;

%reaction rate constant of the Dushman reaction from Arian and Pauer 2021
rateConstants.k2 = 1.37E9*10^(-1.93*sqrt(ionstrength)/(1+sqrt(ionstrength))+0.40*ionstrength);
...
dndt(2) = dg * volume.injection/volume.main * initialSubstance.H2BO3 - r1;        %dn(H2BO3-)/dt
dndt(3) = dg * volume.injection/volume.main * initialSubstance.IO3 - r2;          %dn(IO3-)/dt
dndt(4) = dg * volume.injection/volume.main * initialSubstance.I - 5 * r2 - r3;   %dn(I-)/dt
dndt(5) = 3 * r2 - r3;                                                            %dn(I2)/dt
dndt(6) = r3;                                                                     %dn(I3-)/dt
dndt(7) = dg * volume.injection/volume.main * initialSubstance.H3BO3 + r1;        %dn(H3BO3)/dt
return